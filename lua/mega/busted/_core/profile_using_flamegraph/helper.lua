...
function _P.get_allowed_tags_from_environment_variable()
    return vim.fn.split(os.getenv("BUSTED_PROFILER_ALLOWED_TAGS") or ".*", _TAG_SEPARATOR)
end
...
---
---@param name string
---    An environment variable to check. e.g. `"BUSTED_PROFILER_FLAMEGRAPH_OUTPUT_PATH"`.
...
function _P.get_simple_environment_variable_data()
    ...
    return {
        allow_nightly = _get_bool("BUSTED_PROFILER_ALLOW_NIGHTLY", false),
        keep_temporary_files = _get_bool("BUSTED_PROFILER_KEEP_TEMPORARY_FILES", false),
        maximum_tries = _P.get_number_or_more("BUSTED_PROFILER_MAXIMUM_TRIES", 10, 1),
        minimum_samples = _P.get_number_or_more("BUSTED_PROFILER_MINIMUM_SAMPLES", 2, 1),
        root = root,
...
function M.get_standalone_environment_variable_data()
    ...
    local result = vim.tbl_deep_extend("force", options, {
        timing_threshold = _P.get_number_or_more("BUSTED_PROFILER_TIMING_THRESHOLD", 30, 1),
    })
