...
function _P.get_direct_children(event, starting_index, starting_indices, all_events, all_events_count)
    ...
    if starting_index == all_events_count then
        local current = all_events[starting_index]

        if current.ts > event.ts and (current.ts + current.dur) < (event.ts + event.dur) then
