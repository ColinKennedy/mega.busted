...
function _P.add_minimal_environment_variables()
    _P.setenv("BUSTED_PROFILER_FLAMEGRAPH_OUTPUT_PATH", "foo")
end
...
function _P.restore_environment_variables()
...
function _P.save_environment_variables()
...
before_each(function()
    _P.save_environment_variables()
    _P.mock_main()
end)

after_each(function()
    _P.restore_environment_variables()
    _P.restore_main()
end)
...
describe("environment variable validation", function()
    ...
    describe("BUSTED_PROFILER_ALLOW_NIGHTLY", function()
        it("errors if not 0 or 1", function()
            _P.setenv("BUSTED_PROFILER_ALLOW_NIGHTLY", "3")
            _P.assert_test_failure('$BUSTED_PROFILER_ALLOW_NIGHTLY must be 0 or 1. Got "3" number.')

            _P.setenv("BUSTED_PROFILER_ALLOW_NIGHTLY", "ttt")
            _P.assert_test_failure('$BUSTED_PROFILER_ALLOW_NIGHTLY must be 0 or 1. Got "ttt" unknown value.')
        end)
    end)

    describe("BUSTED_PROFILER_MAXIMUM_TRIES", function()
        it("errors if not 1-or-more", function()
            _P.setenv("BUSTED_PROFILER_MAXIMUM_TRIES", "0")
            _P.assert_test_failure('$BUSTED_PROFILER_MAXIMUM_TRIES must be 1-or-more. Got "0" value.')

            _P.setenv("BUSTED_PROFILER_MAXIMUM_TRIES", "ttt")
            _P.assert_test_failure('$BUSTED_PROFILER_MAXIMUM_TRIES must be 1-or-more. Got "ttt" unknown value.')
        end)
    end)

    describe("BUSTED_PROFILER_MINIMUM_SAMPLES", function()
        it("errors if not 1-or-more", function()
            _P.setenv("BUSTED_PROFILER_MINIMUM_SAMPLES", "0")
            _P.assert_test_failure('$BUSTED_PROFILER_MINIMUM_SAMPLES must be 1-or-more. Got "0" value.')

            _P.setenv("BUSTED_PROFILER_MINIMUM_SAMPLES", "ttt")
            _P.assert_test_failure('$BUSTED_PROFILER_MINIMUM_SAMPLES must be 1-or-more. Got "ttt" unknown value.')
        end)
    end)

    describe("BUSTED_PROFILER_TIMING_THRESHOLD", function()
        it("errors if not 1-or-more", function()
            _P.setenv("BUSTED_PROFILER_TIMING_THRESHOLD", "0")
            _P.assert_test_failure('$BUSTED_PROFILER_TIMING_THRESHOLD must be 1-or-more. Got "0" value.')

            _P.setenv("BUSTED_PROFILER_TIMING_THRESHOLD", "ttt")
            _P.assert_test_failure('$BUSTED_PROFILER_TIMING_THRESHOLD must be 1-or-more. Got "ttt" unknown value.')
        end)
    end)

    describe("BUSTED_PROFILER_KEEP_TEMPORARY_FILES", function()
        it("errors if not 0 or 1", function()
            _P.setenv("BUSTED_PROFILER_KEEP_TEMPORARY_FILES", "2")
            _P.assert_test_failure('$BUSTED_PROFILER_KEEP_TEMPORARY_FILES must be 0 or 1. Got "2" number.')

            _P.setenv("BUSTED_PROFILER_KEEP_TEMPORARY_FILES", "-1")
            _P.assert_test_failure('$BUSTED_PROFILER_KEEP_TEMPORARY_FILES must be 0 or 1. Got "-1" number.')

            _P.setenv("BUSTED_PROFILER_KEEP_TEMPORARY_FILES", "ttt")
            _P.assert_test_failure('$BUSTED_PROFILER_KEEP_TEMPORARY_FILES must be 0 or 1. Got "ttt" unknown value.')
        end)
    end)
end)
